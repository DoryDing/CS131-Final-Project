...
from output.diagram import draw_formation  # fixed: no sys.path hack

PINK = "#FF2D78"
DARK_BG = "#1a1a1a"

def make_cover_page(title, formations, member_legend=None):
    ...
    # member legend line  e.g. "4 members: red_hair, white_shoes, ..."
    if member_legend:
        n = len(member_legend)
        labels = ", ".join(member_legend[str(k)] for k in sorted(int(k) for k in member_legend))
        fig.text(0.5, 0.41, f"{n} members: {labels}",
    ...
    if formations and "start_time_s" in formations[0]:
        ...
        fig.text(0.5, 0.33, f"{start}s  –  {end}s",
...
def add_legend(fig, member_ids, member_legend=None):
    """
    Draw dots + labels along the top of the figure.
    If member_legend is provided, shows costume label under each dot.
    """
    start_x = 0.04
    y = 0.91
    dot_spacing = 0.055 if member_legend else 0.035
    ...
    for i, member_id in enumerate(sorted(member_ids)):
        ...
        # costume label below dot
        if member_legend:
            label = member_legend.get(str(member_id), "")
            fig.text(x, y - 0.05, label,
...
def build_pdf(formations, output_path, title="Formation Sheet", member_legend=None):
    ...
    for formation in formations:
        all_member_ids.update(int(k) for k in formation["positions"].keys())

    with PdfPages(output_path) as pdf:
        # page 1: cover
        cover = make_cover_page(title, formations, member_legend=member_legend)
        ...
        for i, formation in enumerate(formations):
            ...
            add_legend(fig, all_member_ids, member_legend=member_legend)
